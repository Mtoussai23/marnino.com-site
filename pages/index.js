            ...
            {useDesktopLayout && (
              <div className="absolute inset-0 z-0 overflow-hidden">
                <Image
                  ...
                  fill
                  priority
                  className={`object-cover will-change-transform ${isMobilePortrait ? "" : "scale-[1.01]"}`}
                />
      ...
      {isHomePage && (
        <>
          ...
          <button
            ...
            className="absolute bottom-5 right-5 w-8 h-8 border border-white rounded-full flex items-center justify-center text-white z-[1000]"
          >
            M
          </button>
          ...
          <AnimatePresence>
            {isFooterOpen && (
              ...
              >
                <a href="https://www.instagram.com/marnino_" className="footer-link" target="_blank" rel="noreferrer">Instagram</a>
                <a href="https://www.tiktok.com/@marninot" className="footer-link" target="_blank" rel="noreferrer">TikTok</a>
                <a href="https://www.facebook.com/MarninoT/" className="footer-link" target="_blank" rel="noreferrer">Facebook</a>
          ...
          <AnimatePresence>
            {isStoriesOpen && (
              <motion.div className="absolute top-16 left-1/2 -translate-x-1/2 border-2 border-white rounded-lg p-6 text-white w-80 shadow-lg z-[1000] backdrop-blur-xl bg-opacity-30">
                <button onClick={() => setStoriesOpen(false)} className="absolute top-2 right-2 text-white text-xl font-bold hover:text-gray-300">✖</button>
                  ...
                  {stories.length ? (
                    stories.map((story) => (
                      <li key={story.id} className="mb-4 pb-4">
                        ...
                        {story.mediaUrl && (
                          <img src={story.mediaUrl} alt="Story" className="w-full h-auto object-cover rounded-lg shadow-md mt-2" />
                        )}
                        <div className="flex space-x-2 mt-2">
                          {emojiReactions.map((emoji) => (
                            <motion.button key={emoji} onClick={() => handleReaction(story.id, emoji)} className="text-xl" whileTap={{ scale: 1.5 }}>
